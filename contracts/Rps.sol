...
contract RPS{
    ...
    function solve(Move _c1, uint256 _salt) public {
        ...
        require(keccak256(abi.encode(_c1,_salt))==c1Hash); // Verify the value is the commited one.
     ...
     *  * NOTE: Function changed from `constant` to `view` to reflect updated Solidity conventions. From ^0.4.. to ^0.8.28
     */
...
contract Hasher{
    ...
    function hash(uint8 _c, uint256 _salt) public pure returns(bytes32) {
        return keccak256(abi.encode(_c,_salt));
    }
}